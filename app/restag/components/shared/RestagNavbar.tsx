        ...
        <div className="flex-1 max-w-[160px] sm:max-w-md">
          <div className={`relative group transition-all duration-500 ${isSearchFocused ? 'scale-105' : ''}`}>
            <div className="absolute inset-y-0 left-3 sm:left-4 flex items-center pointer-events-none">
              <Search className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-colors ${isSearchFocused ? 'text-white' : 'text-gray-500'}`} style={isSearchFocused ? { color: theme.colors.primary } : {}} />
            </div>
            <input
              type="text"
              ...
              className="w-full bg-white/5 border border-white/10 focus:border-white/30 rounded-full py-2 sm:py-2.5 pl-9 sm:pl-11 pr-4 text-[10px] sm:text-xs text-white placeholder:text-gray-600 outline-none transition-all backdrop-blur-xl"
            />
            {/* Input Glow */}
            <div 
...
const MobileMenu = ({ isOpen, onClose, theme, themeName, toggleTheme, pathname }: any) => {
  return (
    <div className={`fixed inset-0 z-[110] md:hidden transition-all duration-500 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      />
      <div 
        className={`absolute bottom-0 left-0 right-0 w-full bg-[#0a0a0a]/95 backdrop-blur-3xl border-t border-white/10 p-8 flex flex-col gap-6 transition-transform duration-500 rounded-t-[40px] ${
      ...
      >
        ...
        <div className="flex justify-between items-center -mx-8 p-6 border-b border-white/10">
