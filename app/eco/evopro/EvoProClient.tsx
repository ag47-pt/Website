...
export default function EvoProClient() {
  ...
  const sections=['overview','problem','cognitive','architecture','host-model','harnesses','continuity','goal','lifecycle','baseline','gauntlet','judge','graph','modes','quickstart','ide-chat','terminal-demo','cli','capabilities','guardrails','observability','use-cases','status','ecosystem'];
  ...
  return <div className="bg-black text-white selection:bg-emerald-500 selection:text-black font-sans relative">
    ...
    <main className="relative z-10">
      <EvoHero/><ProblemComparison/>
      {/* Cognitive Layer: understand before evolution */}
      <CognitiveArchitecture/><DeterministicVsCognitive/><HostModelSection/><HarnessIndependence/><ContinuityFlow/>
      {/* Evolution Engine: act only after governed understanding */}
      <GlobalGoalSection/><EvolutionCycle/><ChangeVsEvolution/><GauntletSection/><JudgeSection/><GraphIntelligence/><ExecutionModes/>
      {/* Usage: agent-first before operator interfaces */}
      <QuickStart/><IdeChatFlow/><TerminalInteractive/><CliReference/>
      {/* Trust / product */}
      <CapabilityMatrix/><GuardrailsSection/><ObservabilitySection/><UseCasesSection/><StatusAndRoadmap/><GitHubEcosystemCTA/>
    </main>
