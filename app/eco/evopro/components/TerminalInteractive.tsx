...
export function TerminalInteractive(){
  const publicState=useEvoProPublic();const version=publicState?.manifest.version||EVOPRO_CONFIG.version;const[active,setActive]=useState(DEMOS[0].id);const[copied,setCopied]=useState(false);const demo=DEMOS.find(x=>x.id===active)||DEMOS[0];
