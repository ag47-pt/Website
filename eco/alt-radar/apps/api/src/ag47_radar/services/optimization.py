...
def evaluate_weight_matrix(
    ...
    for score in scores:
        ...
        if score.momentum_score is not None and score.liquidity_score is not None:
            recalculated = (
                ...
                + 5.0
                * (
                    weights.get("community_score", 0.15)
                    + weights.get("distribution_score", 0.15)
                    + weights.get("safety_score", 0.20)
                    + weights.get("data_quality_score", 0.05)
                )
            )
            if recalculated >= 6.0:
    ...
    profit_factor = (
        round(total_pos / total_neg, 4)
        if total_neg > 0
        else round(total_pos if total_pos > 0 else 1.0, 4)
    )
...
async def run_grid_search_optimization(
    ...
    *,
    ...
    sorted_evals = sorted(
        evaluations, key=lambda x: (x.profit_factor, x.win_rate, x.total_return_pct), reverse=True
    )
