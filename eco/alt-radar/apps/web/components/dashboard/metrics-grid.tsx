...
export function formatSyncTime(lastSyncAt: string | null | undefined, hasError: boolean) {
  if (!lastSyncAt) {
    return {
      time: hasError ? "Indisponível" : "Aguardando",
      full: hasError ? "API indisponível" : "Sem sincronização confirmada",
    };
  }
  ...
  try {
    ...
    return {
      ...
      full: formatDateTime(lastSyncAt),
    };
  } catch {
    return {
      time: "Aguardando",
      full: "Timestamp de sincronização inválido",
    };
  }
}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <p className="truncate text-[0.65rem] font-mono font-bold uppercase tracking-wider text-zinc-400">
              {label}
            </p>
            <span title={help} className="cursor-help">
              <Info
                aria-label={help}
                className="size-3 text-zinc-500 hover:text-zinc-300 transition-colors"
              />
            </span>
          </div>
          {isLoading ? (
          ...
          <p
            className="mt-1 truncate font-mono text-[0.62rem] text-zinc-400"
...
export function MetricsGrid() {
  ...
  const formattedTime = formatSyncTime(status.data?.last_sync_at, status.isError);
  const metricContext = status.isError
    ? "API indisponível"
    : metrics
      ? "Confirmado pela API"
  ...
  const cards: MetricCardProps[] = [
    ...
    {
      label: "Score médio",
      value: metrics?.average_score != null ? `${formatScore(metrics.average_score)}/10` : "N/D",
      comparison: metricContext,
    ...
    },
    {
      ...
      comparison: status.data
        ? `${status.data.providers.length} configurados pela API`
        : metricContext,
    ...
    <section aria-labelledby="overview-title" className="rise rise-1">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <h1
            id="overview-title"
            className="mt-1 text-2xl font-black tracking-tight text-white font-sans"
          >
        ...
        <DataBadges
          ...
          partial={
            ...
            status.data?.monitoring_active === false
          }
        />
      </div>
