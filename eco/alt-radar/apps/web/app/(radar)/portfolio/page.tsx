    ...
    return (
      <div className="flex h-[calc(100vh-4rem)] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-mono text-sm tracking-widest text-radar-muted uppercase">
            Sincronizando Módulo Observacional
          </p>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col gap-8 p-6 md:p-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold tracking-tight text-radar-ink">Paper Trading</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="group relative overflow-hidden rounded-2xl border border-radar-border bg-gradient-to-b from-white/[0.03] to-white/[0.01] p-5 backdrop-blur-xl">
          <div
            className={`absolute inset-0 bg-gradient-to-br opacity-0 transition-opacity duration-500 group-hover:opacity-100 ${isProfit ? "from-radar-positive/5" : "from-red-500/5"} to-transparent`}
          />
          <p className="text-[0.7rem] font-bold uppercase tracking-wider text-radar-subtle">
            PnL Global
          </p>
          <div className="mt-3 flex items-baseline gap-2">
            <span
              className={`text-3xl font-bold tracking-tight ${isProfit ? "text-radar-positive" : "text-red-400"}`}
            >
      ...
      <div className="flex flex-col gap-4">
        <h2 className="text-sm font-semibold tracking-wide text-radar-ink">
          Posições Virtuais ({positions?.length ?? 0})
        </h2>
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-white/[0.04] bg-white/[0.02]">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-semibold text-radar-subtle">
                  Preço Entrada
                </th>
                  ...
                  return (
                    <tr key={pos.id} className="transition-colors hover:bg-white/[0.01]">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center rounded-full px-2 py-0.5 text-[0.65rem] font-bold tracking-wider ${pos.status === "OPEN" ? "bg-radar-positive/10 text-radar-positive ring-1 ring-radar-positive/30" : "bg-white/5 text-radar-subtle"}`}
                        >
                          {pos.status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <span
                          className={`inline-flex items-center gap-1 font-mono ${posProfit ? "text-radar-positive" : "text-red-400"}`}
                        >
                          {posProfit ? (
                          ...
                          <span className="text-xs opacity-60">
                            ({formatPercent(pos.pnl_percentage ?? 0)})
                          </span>
                        </span>
                      </td>
                    </tr>
