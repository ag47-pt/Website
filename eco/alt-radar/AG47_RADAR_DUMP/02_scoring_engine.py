...
def calculate_discrepancy(val1: float | None, val2: float | None) -> float | None:
    """Calculate relative discrepancy ratio between two provider values.

    Returns a float between 0.0 and 1.0 (e.g. 0.25 = 25%), or None if either value is invalid/missing.
    """
    if val1 is None or val2 is None or val1 <= 0 or val2 <= 0:
        return None
    denom = max(val1, val2)
    return round(abs(val1 - val2) / denom, 4)
...
def calculate_score(
    ...
    *,
    ...
    # Cross-validation penalty when DexScreener & GeckoTerminal diverge > 15%
    discrepancy_penalty = 0.0
    if discrepancy_ratio is not None and discrepancy_ratio > 0.15:
    ...
    if discrepancy_ratio is not None and discrepancy_ratio > 0.15:
        negatives.append(
            f"Discrepância de provedores: {discrepancy_ratio * 100:.1f}% em volume/liquidez (excede limiar de 15%)"
        )
