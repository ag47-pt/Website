...
def test_infer_hypotheses():
    s1 = TokenSignal(
        id="sig1", token_id="t1", signal_type="liquidity_volume_expansion", created_at=utc_now(), strength=Decimal("1.0"), confidence=Decimal("1.0")
    )
...
async def test_learning_pipeline(db_session):
    ...
    t = Token(id="t1", chain="solana", contract_address="addr1", symbol="ABC", name="ABC Token", metadata_json={}, source="test", is_demo=False)
    db_session.add(t)
    await db_session.commit()
    
    s = TokenSignal(id="sig2", token_id="t1", signal_type="liquidity_volume_expansion", strength=Decimal("1.0"), confidence=Decimal("1.0"), is_demo=False, created_at=utc_now())
    db_session.add(s)
    await db_session.commit()
    ...
    pair = TradingPair(id="pair1", token_id="t1", pair_address="addr", quote_token="SOL", dex="raydium", source="mock", is_demo=False)
    db_session.add(pair)
    await db_session.commit()
    ...
    snap0 = MarketSnapshot(
        price_usd=Decimal("1.0"), captured_at=hypothesis.created_at,
        source="mock", data_quality="high", is_demo=False, pair_id="pair1"
    )
    # Preço final (T+24 = $1.10) (+10%)
    snap24 = MarketSnapshot(
        price_usd=Decimal("1.10"), captured_at=hypothesis.created_at + timedelta(hours=24),
        source="mock", data_quality="high", is_demo=False, pair_id="pair1"
    )
